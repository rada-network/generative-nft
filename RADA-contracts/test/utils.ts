...
export const deployRadaDescriptor = async (
  deployer?: SignerWithAddress,
  ...
  const radaDescriptorFactory = new RadaDescriptorFactory(
    {
...
export const deployRadaSeeder = async (deployer?: SignerWithAddress): Promise<RadaSeeder> => {
  const factory = new RadaSeederFactory(deployer || (await getSigners()).deployer);

  return factory.deploy();
};

export const deployRadaToken = async (
  deployer?: SignerWithAddress,
  minter?: string,
  descriptor?: string,
  seeder?: string,
  ...
  const signer = deployer || (await getSigners()).deployer;
  const factory = new RadaTokenFactory(signer);

  return factory.deploy(
    minter || signer.address,
    descriptor || (await deployRadaDescriptor(signer)).address,
    seeder || (await deployRadaSeeder(signer)).address,
  );
};


export const populateDescriptor = async (nounsDescriptor: RadaDescriptor): Promise<void> => {
...
/**
 * Return a function used to mint `amount` Nft on the provided `token`
 * @param token The Nft ERC721 token
 ...
 */
export const setTotalSupply = async (token: RadaToken, newTotalSupply: number): Promise<void> => {
