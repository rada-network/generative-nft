...
import { RadaDescriptor } from '../typechain';
import ImageData from '../files/image-data.json';
import { LongestPart } from './types';
import { deployRadaDescriptor, populateDescriptor } from './utils';
...
describe('RadaDescriptor', () => {
  let radaDescriptor: RadaDescriptor;
  ...
  before(async () => {
    radaDescriptor = await deployRadaDescriptor();
    ...
    await populateDescriptor(radaDescriptor);
  });
  ...
  it('should generate valid token uri metadata when data uris are enabled', async () => {
    const tokenUri = await radaDescriptor.tokenURI(0, {
      background: 0,
    ...
    expect(name).to.equal('NFT 0');
    expect(description).to.equal('NFT 0 is a member of the NFT RADA');
  ...
  it('should generate valid token uri metadata for all supported parts when data uris are enabled', async () => {
    ...
    for (let i = 0; i < max; i++) {
      const tokenUri = await radaDescriptor.tokenURI(i, {
      ...
      expect(name).to.equal(`NFT ${i}`);
      expect(description).to.equal(`NFT ${i} is a member of the NFT RADA`);
      expect(image).to.not.be.undefined;
